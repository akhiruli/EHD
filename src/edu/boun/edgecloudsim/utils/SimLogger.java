...
import edu.boun.edgecloudsim.thirdProblem.dag.JobStatus;
...
public class SimLogger {
	public static enum TASK_STATUS {
		CREATED, UPLOADING, PROCESSING, DOWNLOADING, COMLETED, REJECTED_DUE_TO_VM_CAPACITY, REJECTED_DUE_TO_BANDWIDTH, UNFINISHED_DUE_TO_BANDWIDTH, UNFINISHED_DUE_TO_MOBILITY, LATENCY_DEADLINE, ENERGY_CONSTRAINT
	}
	...
	private static boolean skipWarmUpPeriod;

	private Map<Integer, Map<Integer, TaskNode>> jobs;

	private Map<Integer, Job> jobMap;
	private Map<Integer, DQNParams> DQNresults;
	private static SimLogger singleton = new SimLogger();
	private HashMap<Integer, List<Double>> actionReward = new HashMap<>();
	...
	public void setTotalSuccTasks(int totalSuccTasks) {
		this.totalSuccTasks = totalSuccTasks;
	}

	private int totalSuccTasks = 0;

	public int getTotalCriticalTaskFailure() {
		return totalCriticalTaskFailure;
	}

	public void setTotalCriticalTaskFailure(int totalCriticalTaskFailure) {
		this.totalCriticalTaskFailure = totalCriticalTaskFailure;
	}

	private int totalCriticalTaskFailure = 0;
	public int getTotalSuccTasks() {
		return totalSuccTasks;
	}
	...
	public double getFailureRate(){
		return ((double) totalFailedTasks * (double) 100)
				/ (double) (totalSuccTasks + totalFailedTasks);
	}
	/*
	...
	private SimLogger() {
		...
		skipWarmUpPeriod = false;
	}

	public static void setSkipWarmUpPeriod(boolean v){
		skipWarmUpPeriod = v;
	}
	...
	public void updateActionReward(Integer action, Double reward){
		if (actionReward.containsKey(action)) {
			actionReward.get(action).add(reward);
		} else {
			List<Double> rewards = new ArrayList<>();
			rewards.add(reward);
			actionReward.put(action, rewards);
		}
	}

	public void printActionReward(){
		String actionR= "";
		for(Map.Entry<Integer, List<Double>> ar : actionReward.entrySet()){
			Integer action = ar.getKey();
			List<Double> rewards = ar.getValue();
			String reward_str = "";
			for(Double reward : rewards){
				reward_str += reward + ",";
			}
			printLine("Action: "+action+" Rewards: "+reward_str);
		}

		SimLogger.printLine("Action Reward: "+actionR);
	}
	...
	public void simStarted(String outFolder, String fileName) {
		...
		DQNresults = new HashMap<>();
	}

	public void saveDQNResult(Integer episode, double reward, double qValue){
		DQNParams dqnParams = new DQNParams(reward, qValue);
		DQNresults.put(episode, dqnParams);
	}

	public void setCritical(int taskId, boolean critical) {
		taskMap.get(taskId).setCritical(critical);
	}
	...
	public void failedDueToLatencyDeadline(int taskId, double time){
		taskMap.get(taskId).taskFailedDueToLatencyDeadline(time);
	}

	public void failedDueToEnergyofUE(int taskId, double time){
		taskMap.get(taskId).taskfailedDueToEnergyofUE(time);
	}
	...
	public void simStopped() throws IOException {
		...
		int failedDueToLatency = 0;
		int failedDueToEnergy = 0;
		...
		for (Map.Entry<Integer, LogItem> entry : taskMap.entrySet()) {
			...
			if (!skipWarmUpPeriod && value.isInWarmUpPeriod())
				continue;
			...
			} else if (value.getStatus() == TASK_STATUS.LATENCY_DEADLINE){
				failedDueToLatency++;
				if(value.isCritical()){
					totalCriticalTaskFailure++;
				}
			} else if (value.getStatus() == TASK_STATUS.ENERGY_CONSTRAINT) {
				failedDueToEnergy++;
				if(value.isCritical()){
					totalCriticalTaskFailure++;
				}
		...
		printLine("# of failed tasks due to latency: " + failedDueToLatency);
		printLine("# of failed tasks due to Energy constraint: " + failedDueToEnergy);
		...
		printLine("# of critical task failed: "+totalCriticalTaskFailure);
		...
		this.printJobStats(false);
		totalFailedTasks=failedTask[numOfAppTypes];
		totalSuccTasks=completedTask[numOfAppTypes];
		// clear related collections (map list etc.)
		taskMap.clear();
		vmLoadList.clear();
	}

	boolean isJobSucceeded(Job job){
		Map<Integer, TaskNode> taskMap = job.getTaskMap();
		for(Map.Entry<Integer, TaskNode> entry : taskMap.entrySet()){
			TaskNode taskNode = entry.getValue();
			if(taskNode.getStatus() != TASK_STATUS.COMLETED)
				return false;
		}
		return true;
	}

	private void printJobStats(boolean debug){
		double totalDelay = 0;
		double totalDelaySucc = 0;
		int count = 0;
		double totalEnergy = 0;
		double totalCpu = 0;
		int totalJobs = 0;
		for(Map.Entry<Integer, Job> entry : jobMap.entrySet()){
			if(entry.getValue().getJobStatus() == JobStatus.COMPLETED) {
				double serviceTime  = entry.getValue().getJobEndTime() - entry.getValue().getJobStartTime();
				if (debug) {
					printLine("Job Id: " + entry.getValue().getJobID() + " service Time: " + serviceTime);
					printLine("Job Id: " + entry.getValue().getJobID() + " Energy consumption: " + entry.getValue().getMobileVM().getEnergyModel().getEnergyConsumptionTillNow() + " Battery left: "+entry.getValue().getMobileVM().getEnergyModel().getBatteryLevel());
				}
				totalDelay += serviceTime;
				totalEnergy += entry.getValue().getMobileVM().getEnergyModel().getEnergyConsumptionTillNow();
				totalCpu += entry.getValue().getMobileVM().getEnergyModel().getCpuEnergyConsumptionTillNow();
				totalJobs++;

				if(isJobSucceeded(entry.getValue())){
					totalDelaySucc += serviceTime;
					count++;
				}
			}
		}
		...
		int numSuccTask = 0;
		double succServiceTime = 0;
		for (Map.Entry<Integer, LogItem> entry : taskMap.entrySet()) {
			Integer key = entry.getKey();
			LogItem value = entry.getValue();
			if (value.getStatus() == SimLogger.TASK_STATUS.COMLETED) {
				numSuccTask++;
				succServiceTime += value.getServiceTime() + value.getNetworkDelay();
			}
		}

		printLine("Total Completed Jobs: " + totalJobs);
		printLine("Total Service Time: "+totalDelay);
		printLine("Average service time of jobs: " + totalDelay/totalJobs);
		printLine("Total Service Time for successful jobs: "+totalDelaySucc);
		printLine("Average service time for successful jobs: "+totalDelay/(double) count);
		printLine("Successful applications: "+ count);
		printLine("Total Energy: "+totalEnergy);

		printLine("Total Energy Consumption: (Total/CPU/NW) "+totalEnergy+"/"+totalCpu+"/"+(totalEnergy-totalCpu));
		printLine("Average Energy Consumption of jobs: " + totalEnergy/totalJobs);
		printLine("Total service time for successful tasks(numSucc/ServiceTime/Average):"+numSuccTask+"/"+succServiceTime+"/"+(succServiceTime/numSuccTask));
	}

	public void setJobMap(Map<Integer, Job> jobMap) {
		this.jobMap = jobMap;
	}
	public void setJobs(Map<Integer, Map<Integer, TaskNode>> jobs) {
		this.jobs = jobs;
	}

	public void setStartTime(int jobId, int taskId, double time){
		TaskNode taskNode = jobs.get(jobId).get(taskId);
		taskNode.setTaskStartTime(time);
		if(taskNode.isStartTask()){
			//System.out.println("Start Time: "+taskId+" JobId: "+jobId+" Time: "+time);
			jobMap.get(jobId).setJobStartTime(time);
			jobMap.get(jobId).setJobStatus(JobStatus.RUNNING);
		}
	}

	public void setEndTime(int jobId, int taskId, double time){
		TaskNode taskNode = jobs.get(jobId).get(taskId);
		taskNode.setTaskEndTime(time);
		if(taskNode.isEndTask()){
			//System.out.println("End Time: "+taskId+" JobId: "+jobId+" Time: "+time);
			jobMap.get(jobId).setJobEndTime(time);
			jobMap.get(jobId).setJobStatus(JobStatus.COMPLETED);
		}
	}

	public void setTaskUploadDelay(int jobId, int taskId, double time){
		jobs.get(jobId).get(taskId).setUploadDelay(time);
	}
}
...
class DQNParams{
	double reward;
	double qValue;
	public DQNParams(double reward, double qValue){
		this.reward = reward;
		this.qValue = qValue;
	}
};
...
class LogItem {
	...
	public void taskFailedDueToLatencyDeadline(double time) {
		taskEndTime = time;
		status = SimLogger.TASK_STATUS.LATENCY_DEADLINE;
		//double processingTime = getServiceTime()-getNetworkDelay();
		//SimLogger.printLine("Akhirul: "+processingTime+ " Nw: "+getNetworkDelay());
	}

	public void taskfailedDueToEnergyofUE(double time) {
		taskEndTime = time;
		status = SimLogger.TASK_STATUS.ENERGY_CONSTRAINT;
	}
	...
	public void setEnergyConsumption(double energyCons){energyConsumption=energyCons;}
