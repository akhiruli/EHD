...
import edu.boun.edgecloudsim.thirdProblem.mobileDevice.EhdParam;
...
public class EhdMobileServerManager extends MobileServerManager{
	private Document edgeDevicesDoc = null;
	private int numOfMobileDevices=0;
	private List<EhdParam> ehdParams;

	public EhdMobileServerManager(int _numOfMobileDevices) {
		numOfMobileDevices=_numOfMobileDevices;
		ehdParams = new ArrayList<>();
	}
	...
	public void createVmList(int brockerId) {
		this.parseEdgeDevicesXML("/Users/akhirul.islam/EdgeCloudSim-DeepEdge/scripts/thirdProblem/config/mobile_devices.xml");
		...
		for (int i = 0; i < numOfMobileDevices; i++) {
			...
			EhdParam ehdParam = ehdParams.get((int) Helper.getRandomInteger(0, ehdParams.size()-1));
			//VM Parameters		
			//MobileVM vm = new MobileVM(vmCounter, brockerId, mips, numOfCores, ram, bandwidth, storage, vmm, new CloudletSchedulerTimeShared());
			EhdMobileVM ehdMobileVM = new EhdMobileVM(vmCounter, brockerId, mips, numOfCores, ram, bandwidth, storage, vmm, new CloudletSchedulerTimeShared(), ehdParam);
			vmList.get(i).add(ehdMobileVM);
			vmCounter++;
		}

		//SimLogger.printLine("vm list size:" +vmList.size());
	}
	...
	private List<MobileHost> createHosts(){
		...
		SimLogger.printLine("Number of hosts: "+hostList.size());
	...
	private void parseEdgeDevicesXML(String filePath){
		try {
			java.io.File devicesFile = new File(filePath);
			DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
			DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
			edgeDevicesDoc = dBuilder.parse(devicesFile);
			edgeDevicesDoc.getDocumentElement().normalize();

			NodeList deviceList = edgeDevicesDoc.getElementsByTagName("device");
			SimLogger.printLine("device list size: "+deviceList.getLength());
			for (int i = 0; i < deviceList.getLength(); i++) {
				Node deviceNode = deviceList.item(i);
				Element deviceElement = (Element) deviceNode;
				String conn = getElement(deviceElement, "connectivity");
				String capacity = getElement(deviceElement, "batteryCapacity");
				String idle = getElement(deviceElement, "idleConsumption");
				String max = getElement(deviceElement, "maxConsumption");
				String mob = getElement(deviceElement, "mobility");

				boolean isEhd = (Helper.getRandomInteger(0,1) == 1)?true:false;
				EhdParam ehdParam = new EhdParam(conn, Double.parseDouble(capacity), Double.parseDouble(idle),
						Double.parseDouble(max), Boolean.parseBoolean(mob), isEhd);
				ehdParams.add(ehdParam);
				//SimLogger.printLine("conn: " + conn + " capacity: "+capacity+" idle: "+idle+" max: "+max+" mob: "+mob);
			}
		}catch (Exception e) {
	...
	private String getElement(Element element, String key) {
		try {
